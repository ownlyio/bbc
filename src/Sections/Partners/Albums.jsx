  ...
  return (
    <Wrapper>
      ...
      {Object.entries(items).map((partners) => {
        return partners[1].length !== 0 && (
          <PartnersDiv>
          {partners[1].map((partner) =>  <LogoBox key={partner} background="#fff" border src={IconModules[partner]} width={sizes[partners[0]]} padding='10px'/>)}
          </PartnersDiv>
        )
      })}
    </Wrapper>
